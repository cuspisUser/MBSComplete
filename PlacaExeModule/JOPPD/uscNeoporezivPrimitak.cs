...
namespace JOPPD
{
    public partial class uscNeoporezivPrimitak : Controls.BaseUserControl
    {
        #region Svojstva
        ...
        private bool SaveData()
        {
            ...
            {
                if (save.Insert("JOPPDOznakaNeoporezivogPrimitka"))
                {
            ...
            {
                if (save.Update("JOPPDOznakaNeoporezivogPrimitka"))
                {
                    return true;
                }
            }
        ...
        public StringBuilder ValidateDataInput()
        {
            ...
            else if (BusinessLogic.pOznaka.Length > 2)
            {
                message.Append(" - Oznaka može sadržavati maksimalno 2 znaka");
            }
            if (BusinessLogic.pKratkiOpis.Length == 0)
            {
                message.Append(" - Kratki opis je obavezno polje");
            }
            else if (BusinessLogic.pKratkiOpis.Length > 110)
            {
                message.Append(" - Kratki opis može sadržavati maksimalno 110 znakova");
            }
            if (BusinessLogic.pDugiOpis.Length == 0)
            {
                message.Append(" - Dugi opis je obavezno polje");
            }
            else if (BusinessLogic.pDugiOpis.Length > 700)
            {
                message.Append(" - Dugi opis može sadržavati maksimalno 700 znakova");
            }
        ...
        private void btnSpremiZatvori_Click(object sender, EventArgs e)
        {
            ...
            if (message.Length == 0)
            {
                ...
                {
                    lblValidationMessages.Text = "Dogodila se greška prilikom upisa neoporezivog primitka.\nKontaktirajte administratora";
                }
            }
            else
            {
