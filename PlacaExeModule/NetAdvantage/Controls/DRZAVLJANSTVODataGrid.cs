namespace NetAdvantage.Controls
{
    ...
    public class DRZAVLJANSTVODataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private DRZAVLJANSTVODataSet dsDRZAVLJANSTVODataSet1 = new DRZAVLJANSTVODataSet();
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsDRZAVLJANSTVODataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsDRZAVLJANSTVODataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsDRZAVLJANSTVODataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsDRZAVLJANSTVODataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsDRZAVLJANSTVODataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsDRZAVLJANSTVODataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "Državljanstvo : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IDRZAVLJANSTVODataAdapter DataAdapter
        {
            get
            {
        ...
        [Browsable(false), Category("Deklarit Work With ")]
        public virtual DRZAVLJANSTVODataSet DataSet
        {
            get
            {
                return this.dsDRZAVLJANSTVODataSet1;
            }
            set
            {
                this.dsDRZAVLJANSTVODataSet1 = value;
            }
        }
