namespace NetAdvantage.Controls
{
    ...
    public class TIPIZNOSADataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private TIPIZNOSADataSet dsTIPIZNOSADataSet1 = new TIPIZNOSADataSet();
        ...
        private void BindDataGrid()
        {
            ...
            this.dv1.Table = this.dsTIPIZNOSADataSet1.TIPIZNOSA;
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsTIPIZNOSADataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsTIPIZNOSADataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsTIPIZNOSADataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsTIPIZNOSADataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsTIPIZNOSADataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsTIPIZNOSADataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "Pregled uplatnih raeuna doprinosa : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual ITIPIZNOSADataAdapter DataAdapter
        {
            get
            {
                ...
                if (DataAdapterFactory.Provider != null)
                {
                    return DataAdapterFactory.GetTIPIZNOSADataAdapter();
                }
                return null;
            }
        }
        ...
        [Category("Deklarit Work With "), Browsable(false)]
        public virtual TIPIZNOSADataSet DataSet
        {
            get
            {
                return this.dsTIPIZNOSADataSet1;
            }
            set
            {
                this.dsTIPIZNOSADataSet1 = value;
            }
        }
