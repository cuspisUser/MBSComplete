namespace NetAdvantage.Controls
{
    ...
    public class KRIZNIPOREZDataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private KRIZNIPOREZDataSet dsKRIZNIPOREZDataSet1 = new KRIZNIPOREZDataSet();
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsKRIZNIPOREZDataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsKRIZNIPOREZDataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsKRIZNIPOREZDataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsKRIZNIPOREZDataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsKRIZNIPOREZDataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsKRIZNIPOREZDataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "KRIZNIPOREZ : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IKRIZNIPOREZDataAdapter DataAdapter
        {
            get
            {
                ...
                if (DataAdapterFactory.Provider != null)
                {
                    return DataAdapterFactory.GetKRIZNIPOREZDataAdapter();
                }
                return null;
            }
        }
        ...
        [Browsable(false), Category("Deklarit Work With ")]
        public virtual KRIZNIPOREZDataSet DataSet
        {
            get
            {
                return this.dsKRIZNIPOREZDataSet1;
            }
            set
            {
                this.dsKRIZNIPOREZDataSet1 = value;
            }
        }
