namespace NetAdvantage.Controls
{
    ...
    public class DOKUMENTDataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private DOKUMENTDataSet dsDOKUMENTDataSet1 = new DOKUMENTDataSet();
        private System.Data.DataView dv1;
        private int m_FillByIDTIPDOKUMENTAIDTIPDOKUMENTA;
        ...
        public DOKUMENTDataGrid()
        {
            this.BindDataGrid();
            DeklaritDataGrid.myFillMethods = new TypeConverter.StandardValuesCollection(new string[] { "Fill", "FillByIDTIPDOKUMENTA" });
        ...
        private void BindDataGrid()
        {
            ...
            this.dv1.Table = this.dsDOKUMENTDataSet1.DOKUMENT;
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsDOKUMENTDataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsDOKUMENTDataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsDOKUMENTDataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsDOKUMENTDataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsDOKUMENTDataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsDOKUMENTDataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "DOKUMENT : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IDOKUMENTDataAdapter DataAdapter
        {
            get
            {
        ...
        [Browsable(false), Category("Deklarit Work With ")]
        public virtual DOKUMENTDataSet DataSet
        {
            get
            {
                return this.dsDOKUMENTDataSet1;
            }
            set
            {
                this.dsDOKUMENTDataSet1 = value;
            }
        }
