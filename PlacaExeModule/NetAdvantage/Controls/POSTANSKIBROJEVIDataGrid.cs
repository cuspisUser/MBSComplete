namespace NetAdvantage.Controls
{
    ...
    public class POSTANSKIBROJEVIDataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private POSTANSKIBROJEVIDataSet dsPOSTANSKIBROJEVIDataSet1 = new POSTANSKIBROJEVIDataSet();
        ...
        public POSTANSKIBROJEVIDataGrid()
        {
            this.BindDataGrid();
        ...
        private void BindDataGrid()
        {
            ...
            this.dv1.Table = this.dsPOSTANSKIBROJEVIDataSet1.POSTANSKIBROJEVI;
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsPOSTANSKIBROJEVIDataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsPOSTANSKIBROJEVIDataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsPOSTANSKIBROJEVIDataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsPOSTANSKIBROJEVIDataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsPOSTANSKIBROJEVIDataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsPOSTANSKIBROJEVIDataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "POSTANSKIBROJEVI : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IPOSTANSKIBROJEVIDataAdapter DataAdapter
        {
            get
            {
                ...
                if (DataAdapterFactory.Provider != null)
                {
                    return DataAdapterFactory.GetPOSTANSKIBROJEVIDataAdapter();
                }
                return null;
            }
        }
        ...
        [Category("Deklarit Work With "), Browsable(false)]
        public virtual POSTANSKIBROJEVIDataSet DataSet
        {
            get
            {
                return this.dsPOSTANSKIBROJEVIDataSet1;
            }
            set
            {
                this.dsPOSTANSKIBROJEVIDataSet1 = value;
            }
        }
