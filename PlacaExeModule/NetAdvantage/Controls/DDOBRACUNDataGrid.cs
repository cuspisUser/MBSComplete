namespace NetAdvantage.Controls
{
    ...
    public class DDOBRACUNDataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private DDOBRACUNDataSet dsDDOBRACUNDataSet1 = new DDOBRACUNDataSet();
        ...
        private void BindDataGrid()
        {
            ...
            this.dv1.Table = this.dsDDOBRACUNDataSet1.DDOBRACUN;
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsDDOBRACUNDataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsDDOBRACUNDataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsDDOBRACUNDataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsDDOBRACUNDataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsDDOBRACUNDataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsDDOBRACUNDataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "DDOBRACUN : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IDDOBRACUNDataAdapter DataAdapter
        {
            get
            {
                ...
                if (DataAdapterFactory.Provider != null)
                {
                    return DataAdapterFactory.GetDDOBRACUNDataAdapter();
                }
                return null;
            }
        }
        ...
        public virtual DDOBRACUNDataSet DataSet
        {
            get
            {
                return this.dsDDOBRACUNDataSet1;
            }
            set
            {
                this.dsDDOBRACUNDataSet1 = value;
            }
        }
