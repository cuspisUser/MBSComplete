namespace NetAdvantage.Controls
{
    ...
    public class RAD1SPREMEDataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private RAD1SPREMEDataSet dsRAD1SPREMEDataSet1 = new RAD1SPREMEDataSet();
        ...
        private void BindDataGrid()
        {
            ...
            this.dv1.Table = this.dsRAD1SPREMEDataSet1.RAD1SPREME;
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsRAD1SPREMEDataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsRAD1SPREMEDataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsRAD1SPREMEDataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsRAD1SPREMEDataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsRAD1SPREMEDataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsRAD1SPREMEDataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "RAD1SPREME : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IRAD1SPREMEDataAdapter DataAdapter
        {
            get
            {
                ...
                if (DataAdapterFactory.Provider != null)
                {
                    return DataAdapterFactory.GetRAD1SPREMEDataAdapter();
                }
                return null;
            }
        }
        ...
        [Browsable(false), Category("Deklarit Work With ")]
        public virtual RAD1SPREMEDataSet DataSet
        {
            get
            {
                return this.dsRAD1SPREMEDataSet1;
            }
            set
            {
                this.dsRAD1SPREMEDataSet1 = value;
            }
        }
