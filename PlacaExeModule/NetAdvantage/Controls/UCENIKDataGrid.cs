namespace NetAdvantage.Controls
{
    ...
    public class UCENIKDataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private UCENIKDataSet dsUCENIKDataSet1 = new UCENIKDataSet();
        private System.Data.DataView dv1;
        private bool m_FillByPage = false;
        private string m_FillByPOSTANSKIBROJPOSTANSKIBROJ;
        ...
        public UCENIKDataGrid()
        {
            this.BindDataGrid();
            DeklaritDataGrid.myFillMethods = new TypeConverter.StandardValuesCollection(new string[] { "Fill", "FillByPOSTANSKIBROJ" });
        ...
        private void BindDataGrid()
        {
            ...
            this.dv1.Table = this.dsUCENIKDataSet1.UCENIK;
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsUCENIKDataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsUCENIKDataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsUCENIKDataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsUCENIKDataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsUCENIKDataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsUCENIKDataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "UCENIK : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IUCENIKDataAdapter DataAdapter
        {
            get
            {
                ...
                if (DataAdapterFactory.Provider != null)
                {
                    return DataAdapterFactory.GetUCENIKDataAdapter();
                }
                return null;
            }
        }
        ...
        [Browsable(false), Category("Deklarit Work With ")]
        public virtual UCENIKDataSet DataSet
        {
            get
            {
                return this.dsUCENIKDataSet1;
            }
            set
            {
                this.dsUCENIKDataSet1 = value;
            }
        }
        ...
        public virtual string FillByPOSTANSKIBROJPOSTANSKIBROJ
        {
            get
            {
                return this.m_FillByPOSTANSKIBROJPOSTANSKIBROJ;
            }
            set
            {
                this.m_FillByPOSTANSKIBROJPOSTANSKIBROJ = value;
            }
        }
