namespace NetAdvantage.Controls
{
    ...
    public class URAVRSTAIZNOSADataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private URAVRSTAIZNOSADataSet dsURAVRSTAIZNOSADataSet1 = new URAVRSTAIZNOSADataSet();
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsURAVRSTAIZNOSADataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsURAVRSTAIZNOSADataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsURAVRSTAIZNOSADataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsURAVRSTAIZNOSADataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsURAVRSTAIZNOSADataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsURAVRSTAIZNOSADataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "Vrste iznosa URA : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IURAVRSTAIZNOSADataAdapter DataAdapter
        {
            get
            {
                ...
                if (DataAdapterFactory.Provider != null)
                {
                    return DataAdapterFactory.GetURAVRSTAIZNOSADataAdapter();
                }
                return null;
            }
        }
        ...
        [Browsable(false), Category("Deklarit Work With ")]
        public virtual URAVRSTAIZNOSADataSet DataSet
        {
            get
            {
                return this.dsURAVRSTAIZNOSADataSet1;
            }
            set
            {
                this.dsURAVRSTAIZNOSADataSet1 = value;
            }
        }
