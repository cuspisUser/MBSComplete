namespace NetAdvantage.Controls
{
    ...
    public class LOKACIJEDataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private LOKACIJEDataSet dsLOKACIJEDataSet1 = new LOKACIJEDataSet();
        ...
        private void BindDataGrid()
        {
            ...
            this.dv1.Table = this.dsLOKACIJEDataSet1.LOKACIJE;
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsLOKACIJEDataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsLOKACIJEDataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsLOKACIJEDataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsLOKACIJEDataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsLOKACIJEDataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsLOKACIJEDataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "Lokacije OS-a i SI-a : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual ILOKACIJEDataAdapter DataAdapter
        {
            get
            {
        ...
        public virtual LOKACIJEDataSet DataSet
        {
            get
            {
                return this.dsLOKACIJEDataSet1;
            }
            set
            {
                this.dsLOKACIJEDataSet1 = value;
            }
        }
