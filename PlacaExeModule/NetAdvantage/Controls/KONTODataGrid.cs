namespace NetAdvantage.Controls
{
    ...
    public class KONTODataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private KONTODataSet dsKONTODataSet1 = new KONTODataSet();
        private System.Data.DataView dv1;
        private int m_FillByIDAKTIVNOSTIDAKTIVNOST;
        ...
        public KONTODataGrid()
        {
            this.BindDataGrid();
            DeklaritDataGrid.myFillMethods = new TypeConverter.StandardValuesCollection(new string[] { "Fill", "FillByIDAKTIVNOST" });
        ...
        private void BindDataGrid()
        {
            ...
            this.dv1.Table = this.dsKONTODataSet1.KONTO;
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsKONTODataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsKONTODataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsKONTODataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsKONTODataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsKONTODataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsKONTODataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "Kontni plan : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IKONTODataAdapter DataAdapter
        {
            get
            {
        ...
        [Browsable(false), Category("Deklarit Work With ")]
        public virtual KONTODataSet DataSet
        {
            get
            {
                return this.dsKONTODataSet1;
            }
            set
            {
                this.dsKONTODataSet1 = value;
            }
        }
