namespace NetAdvantage.Controls
{
    ...
    public class JEDINICAMJEREDataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private JEDINICAMJEREDataSet dsJEDINICAMJEREDataSet1 = new JEDINICAMJEREDataSet();
        ...
        private void BindDataGrid()
        {
            ...
            this.dv1.Table = this.dsJEDINICAMJEREDataSet1.JEDINICAMJERE;
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsJEDINICAMJEREDataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsJEDINICAMJEREDataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsJEDINICAMJEREDataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsJEDINICAMJEREDataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsJEDINICAMJEREDataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsJEDINICAMJEREDataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "JEDINICAMJERE : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IJEDINICAMJEREDataAdapter DataAdapter
        {
            get
            {
        ...
        public virtual JEDINICAMJEREDataSet DataSet
        {
            get
            {
                return this.dsJEDINICAMJEREDataSet1;
            }
            set
            {
                this.dsJEDINICAMJEREDataSet1 = value;
            }
        }
