namespace NetAdvantage.Controls
{
    ...
    public class ZAPOSLENIDataGrid : DeklaritDataGrid
    {
        private int currentRow;
        private ZAPOSLENIDataSet dsZAPOSLENIDataSet1 = new ZAPOSLENIDataSet();
        ...
        private void BindDataGrid()
        {
            ...
            this.dv1.Table = this.dsZAPOSLENIDataSet1.ZAPOSLENI;
        ...
        private void FillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    ...
                    {
                        this.DataAdapter.FillPage(this.dsZAPOSLENIDataSet1, 0, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsZAPOSLENIDataSet1, info, 0, this.PageSize);
                    }
                }
                else if (this.FillMethod == "Fill")
                {
                    this.DataAdapter.Fill(this.dsZAPOSLENIDataSet1);
                }
                else
                {
                    MethodInfo info2 = this.DataAdapter.GetType().GetMethod(this.FillMethod);
                    this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsZAPOSLENIDataSet1, info2);
                }
        ...
        private void ReFillDataThread(object state)
        {
            ...
            try
            {
                if (this.FillByPage)
                {
                    if (this.FillMethod == "Fill")
                    {
                        this.DataAdapter.FillPage(this.dsZAPOSLENIDataSet1, this.startRowThread, this.PageSize);
                    }
                    else
                    {
                        MethodInfo info = this.DataAdapter.GetType().GetMethod("FillPageBy" + this.FillMethod.Substring(6));
                        this.InvokeFillByMethod(this.FillMethod, (IDataAdapter) this.DataAdapter, this.dsZAPOSLENIDataSet1, info, this.startRowThread, this.PageSize);
                    }
                }
        ...
        private void ResumeBindingPostThread()
        {
            ...
            this.EndUpdate();
            this.BindDataGrid();
            this.Text = "ZAPOSLENI : " + this.DataView.Count.ToString(CultureInfo.InvariantCulture) + " " + Deklarit.Resources.Resources.Rows;
        ...
        public virtual IZAPOSLENIDataAdapter DataAdapter
        {
            get
            {
                ...
                if (DataAdapterFactory.Provider != null)
                {
                    return DataAdapterFactory.GetZAPOSLENIDataAdapter();
                }
                return null;
            }
        }
        ...
        public virtual ZAPOSLENIDataSet DataSet
        {
            get
            {
                return this.dsZAPOSLENIDataSet1;
            }
            set
            {
                this.dsZAPOSLENIDataSet1 = value;
            }
        }
