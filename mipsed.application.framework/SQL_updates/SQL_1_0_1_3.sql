/****** Object:  StoredProcedure [dbo].[sp_OPZStavke]    Script Date: 30.8.2016. 14:41:36 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[sp_OPZStavke]
...
WHERE   
	   ( KONTO.IDAKTIVNOST > 1)
		...
		And firstTbl.IDPARTNER In 
		(SELECT Distinct firstTable.IDPARTNER AS IDPARTNER FROM    dbo.GKSTAVKA AS firstTable
...
WHERE   (firstTable.GKDATUMVALUTE <= @DatumDo)
...
/****** Object:  StoredProcedure [dbo].[sp_OPZForma]    Script Date: 31.8.2016. 8:39:11 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[sp_OPZForma]

(
	@DatumOD as date,
	@DatumDo as date,
	@nisuNaplaceni as date
)
AS
...
SELECT	
		...
		firstTbl.IDGKSTAVKA As ID,
		firstTbl.BROJDOKUMENTA As BrojIzdanogRacuna,
		firstTbl.DATUMDOKUMENTA As DatumIzdanogRacuna,
		firstTbl.GKDATUMVALUTE AS ValutaPlacanja,
		DATEDIFF(DAY, firstTbl.GKDATUMVALUTE, @nisuNaplaceni) As BrojDanaKasnjenja,
		Round((IRA.IRAUKUPNO - (IRA.PDV05 + IRA.PDV10 + IRA.PDV22 + IRA.PDV23 + IRA.PDV25)), 2) As IznosRacuna,
		Round((IRA.PDV05 + IRA.PDV10 + IRA.PDV22 + IRA.PDV23 + IRA.PDV25), 2) As IznosPDVa,
		Round(IRA.IRAUKUPNO, 2) As UkupanIznosSaPDVom,
		Round((Select ISNULL(SUM(ZATVARANJAIZNOS), 0) From ZATVARANJA
		Inner Join GKSTAVKA On ZATVARANJA.GK1IDGKSTAVKA = GKSTAVKA.IDGKSTAVKA
		Where GKSTAVKA.DATUMDOKUMENTA <= @datumDo And GK2IDGKSTAVKA = firstTbl.IDGKSTAVKA), 2) As PlaceniIznosRacuna,
		Round((IRA.IRAUKUPNO - (Select ISNULL(SUM(ZATVARANJAIZNOS), 0) From ZATVARANJA
		Inner Join GKSTAVKA On ZATVARANJA.GK1IDGKSTAVKA = GKSTAVKA.IDGKSTAVKA
		Where GKSTAVKA.DATUMDOKUMENTA <= @datumDo And GK2IDGKSTAVKA = firstTbl.IDGKSTAVKA)), 2) As NeplaceniDioRacuna
...
WHERE   (firstTable.GKDATUMVALUTE <= @DatumDo )
		...
		Inner Join GKSTAVKA On ZATVARANJA.GK1IDGKSTAVKA = GKSTAVKA.IDGKSTAVKA
		Where GKSTAVKA.DATUMDOKUMENTA <= @DatumDo And GK2IDGKSTAVKA = firstTable.IDGKSTAVKA ) <> IRA.IRAUKUPNO)
		And Round((IRA.IRAUKUPNO - (Select ISNULL(SUM(ZATVARANJAIZNOS), 0) From ZATVARANJA
		Inner Join GKSTAVKA On ZATVARANJA.GK1IDGKSTAVKA = GKSTAVKA.IDGKSTAVKA
		Where GKSTAVKA.DATUMDOKUMENTA <= @datumDo And GK2IDGKSTAVKA = firstTbl.IDGKSTAVKA)), 2) > 0
		
ORDER BY PARTNER.IDPARTNER, firstTbl.BROJDOKUMENTA
GO


/****** Object:  StoredProcedure [dbo].[sp_OPZPartneri]    Script Date: 30.8.2016. 14:42:17 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[sp_OPZPartneri]
(
    @DatumOd As date,
	@DatumDo As date
) AS
